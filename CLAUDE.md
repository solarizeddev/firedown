...
- **QR browser pairing — the web client is unlocked by a SCAN, and what
  ...
  - **Wire format is shared with the web client** (`backup/fd-pair.js`):
    ...
    here**; a one-byte divergence fails every pairing with no diagnostic.
  ...
  - **Not wired: the `firedown://pair/` deep link from a SYSTEM camera app.**
    The QR encodes that scheme (so the payload is self-identifying and the
    format is forward-compatible), but no manifest intent-filter routes it yet
    — the supported path is Cloud → "Pair a browser" → scan, which is what the
    web page's own instructions tell the user to do. Adding the filter means
    routing into `SettingsActivity` the way `EXTRA_OPEN_CLOUD_BACKUP` does.
