...
/**
 * Backs the horizontal "Recently downloaded" strip on the regular
 * home surface. Fixed-size feed (the ViewModel limits to a handful
 * of items) so a plain ListAdapter with {@link DiffUtil} is enough —
 * no Paging3 indirection like the main DownloadFragment list.
 */
public class HomeRecentDownloadsAdapter
        extends ListAdapter<DownloadEntity, HomeRecentDownloadsAdapter.TileViewHolder> {

    public interface OnTileClickListener {
        void onTileClick(DownloadEntity entity, RecyclerView.ViewHolder holder);
    }
    ...
    public HomeRecentDownloadsAdapter(@Nullable OnTileClickListener listener) {
        super(DIFF);
        mListener = listener;
    }
    ...
    public TileViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.item_home_recent_download, parent, false);
    ...
    static class TileViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View v) {
            if (listener != null && boundEntity != null) {
                listener.onTileClick(boundEntity, this);
            }
        }
    }
}
