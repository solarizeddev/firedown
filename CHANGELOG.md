...
- Translate pages: Firefox's built-in on-device translator is now wired up. "Translate page" in the browser menu (or the prompt that appears on a page in another language) opens a From/To picker; the page translates in place and "Show original" restores it. Nothing you read leaves the phone; language files download once from Mozilla the first time a language is used, and Settings → Translations shows them (with delete), the "never translate" sites, and switches for the feature and the prompt. The picker fills in the page's language on its own once detection finishes (and says "Detecting the page's language…" until then), so opening it right after a page loads no longer leaves "From" empty. The offer is now a card above the bottom bar instead of a snackbar — it names the language pair, and its ⋮ menu holds "Always translate <language>", "Never translate <language>" and "Never translate this site". A translate glyph at the end of the address bar is the standing door: quiet on any page in another language, lit while the page shows a translation (tap it for "Show original" and the options). The translate sheet gained the same always/never switches, and Settings → Translations lists the languages set either way, with "Add language"
