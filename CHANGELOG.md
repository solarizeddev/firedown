...
- Translate pages: Firefox's built-in on-device translator is now wired up. "Translate page" in the browser menu (or the prompt that appears on a page in another language) opens a From/To picker; the page translates in place and "Show original" restores it. Nothing you read leaves the phone; language files download once from Mozilla the first time a language is used, and Settings → Translations shows them (with delete), the "never translate" sites, and switches for the feature and the prompt. The picker fills in the page's language on its own once detection finishes (and says "Detecting the page's language…" until then), so opening it right after a page loads no longer leaves "From" empty. The offer is now a card above the bottom bar instead of a snackbar — it names the language pair, and its ⋮ opens the translate sheet, whose options carry "Always translate <language>", "Never translate <language>" and "Never translate <site>" (the site name is shown in full). A translate glyph at the end of the address bar, the reload button's twin, marks a translated page (tap it for "Show original" and the options); an untranslated page shows no glyph — the offer card and the menu row are its doors. "Never translate this site" / "Never translate <language>" now silence the offer for those pages entirely (Chrome's behaviour); "Translate page" in the menu still works there. The translate sheet gained the same always/never switches, and Settings → Translations lists the languages set either way, with "Add language"
- Save snapshot: pages whose content lives in an embedded frame (Springer's ePDF reader, any iframe-hosted viewer) are now archived with the frame's content instead of an empty shell, and an archive no longer redirects the viewer to the network ("Webpage not available" / ERR_CACHE_MISS) when the page carried a no-JavaScript fallback. Tapping a link inside a saved page opens it in the browser. Pages built from web components (shadow DOM) and images from CDNs that check the page's Referer are now archived too, and the page is scrolled through before saving so lazily loaded content (a paginated reader's pages, lazy images) is in the archive — including content inside web components and a paginated viewer that unloads pages as it scrolls (each page's image is kept from the moment the viewer loaded it)
- Captured media: an HLS video no longer appears several times — once per quality the player switched to, and again after each re-fetch on CDNs that re-sign their playlist URLs (JW Player embeds on news sites). Redirects are no longer captured as if they were files, and a player's seek-bar thumbnail track (a WebVTT file of image cues) is no longer captured as a subtitle, which removed the phantom "CC 1" badge on such videos. A video whose poster image cannot be fetched now shows a decoded frame instead of the generic icon. A player embed that names its clip several ways at once (the player's stream plus the page's declared mp4 links — JW Player, Brightcove) now yields one entry with a quality list instead of one per link, and an embed that only knows its upload filename (`6aaaf….mp4`) is titled with the caption the article wrote next to it
- Substack: podcast episodes and article voiceovers are now captured with their title, author, cover and duration as you browse a feed, profile or post page — they used to appear only after pressing play, untitled (named "src") or all carrying the profile's name. Publications on a custom domain get the episode's page title. Paywalled episodes need a signed-in subscriber
